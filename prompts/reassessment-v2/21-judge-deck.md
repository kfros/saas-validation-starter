...
- G3 (Revealed spend / costly labor): >= 3 independent VERIFIED signals of actual spend or performed costly labor from >= 1 eligible category (`paid_tool_or_pilot`, `internal_labor`, `outsourced_labor`, `dedicated_role`). Stated WTP is excluded from this count. Report full category breakdown.
...
Verdict Logic:
- PASS: all six gates PASS for the chosen coherent candidate scope.
- CONDITIONAL PASS: G1 and G5 must PASS; 0 gates FAIL. Every remaining material UNKNOWN gate must have an explicit condition object with `resolution_method: INTERVIEW`. Record `discovery_plan.interview_cap` (<= 8) and `recommended_next_action: LIMITED_CUSTOMER_DISCOVERY`. Material unresolved technical constraints prevent CONDITIONAL PASS.
...
- `policy_version`: "v2"
- `input_commit_or_snapshot`: "3bf758f" (or snapshot ID)
- `recommended_next_action`: "LIMITED_CUSTOMER_DISCOVERY" | "STOP" | "TECHNICAL_CHECK_REQUIRED" | "REPAIR_RESEARCH"
- `discovery_plan`: object with interview_cap <= 8 (when conditional pass)
...
Validate JSON syntax with python -m json.tool ideas/deck-automation/reassessment-v2/output/scorecard.json if needed. One structural repair maximum. Do not start Stage 2 or development.
```
